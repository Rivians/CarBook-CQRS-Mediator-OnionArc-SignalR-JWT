...
namespace CarBook.WebUI.Controllers
{
	public class AdminFeatureController : Controller
	{
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.DeleteAsync($"https://localhost:7143/api/Features/{id}");
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
		...
		[HttpGet]
		public async Task<IActionResult> UpdateFeature(int id) // güncelleme yapılacak feature'in sayfasına o verileri getirmek için gerekli action
		{
			var client = _httpClientFactory.CreateClient();
